...
module.exports = function(opts) {
  ...
  const requestTimeout = opts.requestTimeoutInSeconds || 120;
  const continueProcessingOnError = opts.hasOwnProperty('continueProcessingOnError')
  ...
  function processRecord(record) {
    ...
    if (recordHasFailed(record)) {
      ...
      return Promise.resolve()
        ...
        .then(function() {
          // Create a timeout Promise that will reject with a timeout error after 120 seconds
          ...
          // Race the onProcess Promise against the timeout Promise
          return Promise.race([onProcess(record), timeout$]);
        })
        .catch((err) => {
          throw err;
        })
