...
describe(__filename, function() {
  ...
  function createQueue(overrides, timeout) {
    overrides = overrides || {};
    return retryQueue(
      Object.assign(
        {
  ...
  describe('#Failures in processing', function() {
    // let clock, fakeSetTimeout;
    //
    // beforeEach(() => {
    //   console.log(Date.now());
    //   fakeSetTimeout = sinon.stub(global, "setTimeout", (callback) => {
    //     callback();
    //     return spy;
    //   });
    ...
    it.only('Should throw a timeout error after 100 seconds', function() {
      const timeout = Promise.resolve().then(() => {
        return new Promise(function(resolve) {
          setTimeout(resolve, 4500); // resolve gets called now
        });
      });
      onProcessStub.onCall(0).returns(timeout);
      ...
      return queue
        ...
        .then(function(records) {
          expect(records.length).toEqual(1);
          expect(records[0]).toInclude({ status: 'failed', retryCount: 1 });
          expect(records[0].failureReason).toContain('timeout');
          expect(onProcessStub.callCount).toEqual(1);
        });
    });
